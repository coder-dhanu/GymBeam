...
import { Save, Loader2, CreditCard, Plus, Edit2, Trash2, X, Star } from 'lucide-react';
...
const Membership = () => {
  ...
  const [headerData, setHeaderData] = useState({
    ...
    heroImage: 'https://images.unsplash.com/photo-1540497077202-7c8a3999166f?auto=format&fit=crop&q=80&w=2070',
    bottomNote: '* All membership plans include initial consultation, free assessment, and access to all standard facilities. Taxes may apply.',
    ctaTitle: 'Need a custom plan?',
    ctaDescription: 'We provide specialized corporate packages and high-performance athlete preparatory programs upon request.'
  });
  ...
  const handleItemSubmit = async (e) => {
    ...
    try {
      ...
      setShowModal(false);
      setEditingId(null);
      setItemForm({ title: '', price: '', category: 'Group Training', features: '', order: plans.length, isPopular: false });
      fetchData();
    } catch (error) {
  ...
  const bootstrapPlans = async () => {
    if (!window.confirm('This will populate the database with your default Group and Personal training plans. Continue?')) return;
    setSaving(true);
    try {
      // 1. Set Header
  ...
  return (
    <AdminLayout>
      <div className="mb-10 flex justify-between items-end">
        <div>
          <h1 className="text-4xl font-bold text-white tracking-tight mb-2 uppercase">Membership Manager</h1>
          <p className="text-white/40 text-sm max-w-2xl leading-relaxed">
            Manage training packages, durations, and dynamic pricing for your members.
          </p>
        </div>
        <div className="flex gap-3">
          <button 
            onClick={bootstrapPlans} disabled={saving}
          ...
          <button 
            onClick={() => { setEditingId(null); setItemForm({ title: '', price: '', category: 'Group Training', features: '', order: plans.length, isPopular: false }); setShowModal(true); }}
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-[#111111] p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Hero Background Image URL</label>
                <input 
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Bottom Disclaimer Note</label>
                <textarea 
                  value={headerData.bottomNote} rows="2"
                  onChange={(e) => setHeaderData({...headerData, bottomNote: e.target.value})}
                  className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                ></textarea>
              </div>
              <div className="pt-4 border-t border-white/5 mt-4">
                <h3 className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-3">Custom CTA Section</h3>
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-white/20 uppercase tracking-widest">CTA Title</label>
                    <input 
                      type="text" value={headerData.ctaTitle} 
                      onChange={(e) => setHeaderData({...headerData, ctaTitle: e.target.value})}
                      className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-white/20 uppercase tracking-widest">CTA Description</label>
                    <textarea 
                      value={headerData.ctaDescription} rows="2"
                      onChange={(e) => setHeaderData({...headerData, ctaDescription: e.target.value})}
                      className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                    ></textarea>
                  </div>
                </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          {['Group Training', 'Personal Training'].map(cat => (
            <div key={cat} className="space-y-4">
              <h3 className="text-sm font-bold text-primary tracking-widest uppercase border-b border-primary/20 pb-2">{cat}</h3>
              <div className="grid grid-cols-1 gap-3">
                {plans.filter(p => p.category === cat).map(plan => (
                  <div key={plan.id} className="bg-[#111111] p-4 rounded-xl border border-white/5 flex items-center justify-between group">
                    <div className="flex items-center gap-5">
                      <div className="w-10 h-10 bg-white/5 rounded flex items-center justify-center text-white/50">
                        <span className="text-xs font-bold">{plan.order + 1}</span>
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h4 className="text-white font-bold uppercase">{plan.title}</h4>
                          {plan.isPopular && <Star size={12} className="text-yellow-500 fill-yellow-500" />}
                        </div>
                        <p className="text-primary text-xs font-bold mt-1">₹{plan.price}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(plan)} className="p-2 hover:bg-white/5 rounded text-white/40 hover:text-white transition-colors">
                        <Edit2 size={16} />
                      </button>
                      <button onClick={() => handleDelete(plan.id)} className="p-2 hover:bg-red-500/10 rounded text-white/40 hover:text-red-500 transition-colors">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  </div>
                ...
                {plans.filter(p => p.category === cat).length === 0 && (
                  <p className="text-[10px] text-white/20 uppercase tracking-widest text-center py-4">No {cat} plans added.</p>
                )}
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-5">
          <div className="bg-[#111111] border border-white/10 w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white uppercase">{editingId ? 'Edit Plan' : 'New Plan'}</h2>
            ...
            <form onSubmit={handleItemSubmit} className="p-8 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Membership Type</label>
                  <select 
                ...
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Duration Label</label>
                  <input type="text" value={itemForm.title} required onChange={(e) => setItemForm({...itemForm, title: e.target.value})} className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none" placeholder="e.g. 1 Month" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5 text-left">
                  <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Price (Numeric String)</label>
                  <input type="text" value={itemForm.price} required onChange={(e) => setItemForm({...itemForm, price: e.target.value})} className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none" placeholder="e.g. 2,000" />
                </div>
                <div className="space-y-1.5 text-left">
              ...
              <div className="space-y-1.5 text-left">
                <label className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Features (One per line)</label>
                <textarea value={itemForm.features} onChange={(e) => setItemForm({...itemForm, features: e.target.value})} rows="3" className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none" placeholder="Access to gym&#10;Locker availability"></textarea>
              </div>

              <div className="flex items-center gap-3 py-2">
                <input 
                  type="checkbox" checked={itemForm.isPopular} 
                ...
                />
                <label className="text-[10px] font-bold text-white/60 uppercase tracking-widest">Mark as Most Popular</label>
              </div>
