...
const Specialization = () => {
  ...
  const [headerData, setHeaderData] = useState({
    ...
    heroSubtitle:
      "Expert-led training programs designed for elite performance.",
  });
  ...
  const bootstrapSpecs = async () => {
    if (
      !window.confirm(
        "This will populate your database with the 5 new Specialization pillars. Continue?",
      )
    )
      return;
    setSaving(true);
    try {
      ...
      await setDoc(doc(db, "settings", "specialization"), {
        ...
        heroSubtitle:
          "Thrive with our specialized training domains and expert coaching staff.",
      });
      ...
      const defaultSpecs = [
        {
          ...
          title: "VARIETY OF\nTRAINING",
          description:
            "In terms of fitness and training, everyone is unique. some people need one an one personal training programs, while others just want show up for a fitness class, group training options may be ideal for some and not others, the variety of training is key in creating a great fitness studio business.",
          iconName: "Dumbbell",
          accentIconName: "TrendingUp",
          bgColor: "bg-bg-tert",
          order: 1,
        },
        {
          ...
          title: "THRIVING\nCOMMUNITY",
          description:
            "Human social relationships are essential to health and happiness. The studio is an ideal place to meet both physical and social needs. When we can create a sense of community and connection in our gyms. The key to building community is to increase member engagement as much as possible.",
          iconName: "Users",
        ...
        },
        {
          ...
          title: "WELL-DESIGNED &\nDECENT",
          description:
            "The location and quality of the facility is an interstate part of the fitness business. The facility is clean, tidy and small simple. Our facilities must match and remain consistent with our brand. Growth requires consistency. Loyal membership base.",
          iconName: "PenTool",
        ...
        },
        {
          ...
          title: "FITNESS\nSPECIALIZATION",
          description:
            "Fitness specialization means advanced knowledge in a specific field. Earning your certification in a specialized field of fitness offers many benefits. It enables us to provide a high level of service to customers, from exercise therapy to strength and conditioning.",
        ...
        },
        {
          ...
          description:
            "To stay ahead of the competition, we are bringing a hybrid fitness business with a digital fitness platform. Home fitness is being adopted as an effective and convenient way to exercise in the modern era.",
          iconName: "Zap",
          accentIconName: "Activity",
          bgColor: "bg-primary",
          order: 5,
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="bg-[#111111] p-6 rounded-2xl border border-white/5 space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  value={headerData.headlinePrefix}
                  onChange={(e) =>
                    setHeaderData({
                      ...headerData,
                      headlinePrefix: e.target.value,
                    })
                  }
              ...
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setHeaderData({ ...headerData, heroTitle: e.target.value })
                  }
                  className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none focus:border-primary/50 transition-colors uppercase font-bold"
                />
              </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {specs.map((item) => (
            <div
              key={item.id}
              className="bg-[#111111] p-5 rounded-xl border border-white/5 flex items-center justify-between group"
            >
              <div className="flex items-center gap-5">
                <div
                  className={`w-12 h-12 ${item.bgColor === "bg-primary" ? "bg-primary" : "bg-white/5"} rounded flex items-center justify-center text-white/50`}
                >
                  <span className="text-xs font-bold">{item.order + 1}</span>
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-primary uppercase tracking-[1px]">
                      {item.category}
                    </span>
                  </div>
                  <h3 className="text-white font-bold uppercase text-lg line-clamp-1">
                    {item.title.replace("\n", " ")}
                  </h3>
                </div>
              </div>
          ...
          {specs.length === 0 && (
            <p className="text-center py-10 text-white/20 uppercase tracking-widest text-xs">
              No pillars found. Click "Add New" to start.
            </p>
          )}
        </div>
      </div>
          ...
          <div className="bg-[#111111] border border-white/10 w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl">
            <div className="p-6 border-b border-white/5 flex justify-between items-center">
              <h2 className="text-xl font-bold text-white uppercase">
                {editingId ? "Edit Pillar" : "Add New Pillar"}
              </h2>
              <button
            ...
            <form onSubmit={handleItemSubmit} className="p-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div className="space-y-1.5 text-left">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setItemForm({ ...itemForm, iconName: e.target.value })
                    }
                    className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                  >
                ...
                <div className="space-y-1.5 text-left">
                  ...
                  <select
                    value={itemForm.bgColor}
                    onChange={(e) =>
                      setItemForm({ ...itemForm, bgColor: e.target.value })
                    }
                    className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                  >
              ...
              <div className="space-y-1.5 text-left">
                ...
                <input
                  type="text"
                  value={itemForm.bgImage}
                  onChange={(e) =>
                    setItemForm({ ...itemForm, bgImage: e.target.value })
                  }
                  className="w-full bg-[#0A0A0A] border border-white/10 rounded-lg p-2.5 text-white text-sm focus:outline-none"
                />
              </div>
